...
//Ladder Climb
if(place_meeting_rounded(x,y,par_LADDER) && self.up_held)
    {
        x=par_LADDER.x+16;
        x_speed=0;
        y_speed=2;
        ...
        else if(!place_meeting_rounded(x,y,par_LADDER) && self.up_held)
    {
    y_speed=0;
    state_switch("Air");    
}
...
//Check for GROUND
if(place_meeting_rounded(x,y,par_LADDER) && ((position_meeting_rounded(x,y+1,obj_ramp) || (position_meeting_rounded(x,y+1,PAR_Wall) || (position_meeting_rounded(x,y+5,PAR_Platform)))&& self.down_held)))
{
    state_switch("Air");
}
